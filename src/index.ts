  ...
  try {
    for (const vp of viewports) {
      ...
      const runOne = async (flow: { name: string; steps: Step[] }): Promise<{
        ...
        const visualOpts = typeof checks.visual === 'object' ? checks.visual : {};
        const visual = checks.visual
          ? await checkVisual(page, flow.name, vp.name, {
              baselineDir,
              outputDir,
              store: store ?? undefined,
              algorithm: visualOpts.algorithm,
              ssimThreshold: visualOpts.ssimThreshold,
              aaTolerance: visualOpts.aaTolerance,
              threshold: visualOpts.threshold,
              failRatio: visualOpts.failRatio,
              ignoreRegions: visualOpts.ignoreRegions,
