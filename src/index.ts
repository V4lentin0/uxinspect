...
import type { OfflineResult } from './offline-audit.js';
...
export { auditOfflineBehavior } from './offline-audit.js';
  ...
  try {
    for (const vp of viewports) {
      ...
      const runOne = async (flow: { name: string; steps: Step[] }): Promise<{
        ...
        const offlineR = checks.offline ? await auditOfflineBehavior(page, { url: config.url }).catch(() => undefined) : undefined;
      ...
      for (const r of results) {
        ...
        if (r.offline) offlineResults.push(r.offline);
      }
