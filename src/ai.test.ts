...
test('AIHelper.act auto-heals a cached-but-stale selector and updates cache heals counter', async () => {
  ...
  const ai = new AIHelper();
  await ai.init(page);
  // Prime the cache with a stale CSS selector. Uses the shared LocatorCache
  // class from P2 #25 (post-merge with #26).
  const instruction = 'click Save profile';
  const primeKey = (ai as any).buildKey(instruction) as string;
  (ai as any).cache.put({
    key: primeKey,
    resolvedSelector: '#nonexistent-old',
    confidence: 0.8,
    strategy: 'css',
    verb: 'click',
  });
  const ok = await ai.act(instruction);
  assert.equal(ok, true, 'act should succeed via self-heal');
  const events = ai.getHealEvents();
  assert.equal(events.length, 1);
  assert.equal(events[0]!.failedSelector, 'css=#nonexistent-old');
  // Cache is updated and heals counter incremented (post-heal key is
  // recomputed because invalidate() drops the stale entry first).
  const postKey = (ai as any).buildKey(instruction) as string;
  const cached = (ai as any).cache.get(postKey);
  assert.ok(cached, 'cache entry should exist after heal');
  assert.equal(cached.heals, 1);
  assert.ok(cached.lastHealAt && cached.lastHealAt > 0);
  await driver.close();
});
...
test('AIHelper self-heal can be disabled via options (no heal events on act)', async () => {
  ...
  const instruction = 'click Submit order';
  const primeKey = (ai as any).buildKey(instruction) as string;
  (ai as any).cache.put({
    key: primeKey,
    resolvedSelector: '#ghost',
    confidence: 0.8,
    strategy: 'css',
    verb: 'click',
  });
