...
const argv = await yargs(hideBin(process.argv))
  ...
  .command('run', 'Run an inspection', (y) =>
    y
      ...
      .option('auth-edge', { type: 'string', describe: 'Auth-edge audit: path to JSON with {storageStatePath, loginUrl, protectedUrl, logoutUrl?, csrfTokenSelector?}' })
...
async function runCmd(): Promise<void> {
  ...
  const checks = {
    ...
    authEdge: a['auth-edge']
      ? JSON.parse(await fs.readFile(path.resolve(a['auth-edge']), 'utf8'))
      : undefined,
  };
