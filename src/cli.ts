...
const argv = await yargs(hideBin(process.argv))
  ...
  .command('run', 'Run an inspection', (y) =>
    y
      ...
      .option('concurrency', { type: 'boolean', describe: 'Concurrency audit — detect 2-tab race conditions (silent overwrite, cross-tab logout, session sync)' })
...
async function runCmd(): Promise<void> {
  ...
  const checks = {
    ...
    concurrency: pick(a.concurrency),
  };
